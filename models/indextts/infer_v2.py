...
class IndexTTS2:
    def __init__(
        ...
        # w2v-bert-2.0 仅用于参考音频特征提取（结果有缓存），放 CPU 省约 2GB 显存
        ...
        # load campplus_model — 仅用于参考音频风格提取（结果有缓存），放 CPU 省显存
        ...
        if self.device != "cpu" and torch.cuda.is_available():
            ...
            print(f">> GPU VRAM allocated: {vram_mb:.0f} MB (semantic_model & campplus_model on CPU)")

    @torch.no_grad()
    def get_emb(self, input_features, attention_mask):
        # semantic_model 在 CPU 上，确保输入也在 CPU
        cpu_device = self.semantic_model.device
        input_features = input_features.to(cpu_device)
        attention_mask = attention_mask.to(cpu_device)
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        if self.cache_spk_cond is None or self.cache_spk_audio_prompt != spk_audio_prompt:
            ...
            # semantic_model 在 CPU，get_emb 内部会处理设备；返回 CPU 张量后转 GPU
            ...
            # campplus_model 在 CPU，feat 需转 CPU；style 转回 GPU
            style = self.campplus_model(feat.unsqueeze(0).to("cpu")).to(self.device)  # 参考音频的全局style2[1,192]
